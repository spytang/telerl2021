...
（1）在奖励函数塑造层面：将风险规避思想凝练为频域打孔负载均衡（FPLB）。现有调度方案多以最小化系统级 eMBB 中断率为单一优化目标。然而，在复杂流量分布下，此类策略可能形成局部资源过度使用，即持续偏向少数具有高信道裕度的 eMBB 链路，进而造成用户级公平性失衡与尾部体验劣化。本文借鉴 Alsenwi 等人\cite{alsenwi2024distributed}在多智能体连续资源分配中的方差控制思想，并在纯离散抢占决策 MDP 框架内提出面向风险规避的方差惩罚策略优化算法（VarPPO）。其核心不是泛化地惩罚所有吞吐波动，而是在奖励函数中显式约束不同频域资源之间的打孔计数方差与剩余可靠性裕度方差，引导智能体在可用资源池内更均衡地分摊物理层打孔效应；码字吞吐状态与滚动吞吐方差则作为辅助稳定性指标，用于抑制由局部过度打孔引发的短期吞吐抖动。该机制旨在不增加多智能体架构复杂度的前提下，在保障 URLLC 时延约束的同时改善 eMBB 侧可靠性风险分布。
...
\end{equation}
其中概率比 \(r_t(\theta)=\pi_\theta(a_t \mid s_t)/\pi_{\theta_{\mathrm{old}}}(a_t \mid s_t)\) 衡量了策略更新前后的动作概率变化，裁剪超参数 $\epsilon$ 用于限制更新幅度。当概率比超出 $[1-\epsilon,1+\epsilon]$ 的范围且该更新对目标函数具有正向贡献时，梯度将被裁剪，从而抑制策略向极端方向过度更新，提升训练过程的稳定性。
...
在每个 Slot 开始时（即当 \(\text{slot\_idx}(t)\) 发生变化时），环境为每个子载波 \(f \in \{0,1,\ldots,F-1\}\) 独立采样一个更广泛的整数容忍度，即
\begin{equation}
...
URLLC 数据包到达过程建模为参数 \(\lambda\) 的泊松过程，即在时步 \(t\) 的到达量满足 \(A(t) \sim \text{Poisson}(\lambda)\)，基础训练中设定为 \(\lambda = 0.5\)。为测试算法在越界到达率（Out-of-Distribution Arrival Rates）下的泛化能力与鲁棒性，本章对 URLLC 的时延容忍度约束进行了重新校准。在母本基线模型中，\(1\,\text{ms}\) 的时隙被划分为 \(14\) 个 Mini-slot，其设定的排队时延上限为 \(7\) 个 Mini-slot（对应 \(0.5\,\text{ms}\)）。然而，根据 \(3\text{GPP}\) 规范，URLLC 服务的端到端（E2E）时延红线为严格的 \(1\,\text{ms}\)。此 \(1\,\text{ms}\) 预算需被传播延迟、传输延迟、排队延迟及软硬件处理延迟共同瓜分\cite{truong2026}。考虑到在实际架构中，基带处理（如编码、解码）通常需占用 \(3\) 个以上的 OFDM 符号时间\cite{esswie2018}，若在基站排队调度阶段即消耗 \(0.5\,\text{ms}\)，将极大增加整体 E2E 时延超标的风险。

因此，为更真实地还原 \(5\text{G NR}\) 物理层对超低时延的严苛要求，本章将每个新到达数据包的初始剩余排队时限（Remaining Deadline）严格缩紧为 \(d_{\text{init}} = D = 3\) 个 Mini-slot（约 \(0.21\,\text{ms}\)）。新包以先进先出（FIFO）顺序进入等待队列 \(\mathcal{Q}(t)\)。这一修改为后续的空口传输及潜在的重传机制预留了充足的时间预算，但也使得系统在发生突发流量时，留给调度器平滑 eMBB 资源被抢占影响的时间窗口急剧缩小。
...
在算法工程实现层面，本章基于 Stable-Baselines3 框架采用双分支 Actor-Critic 网络架构，Actor 与 Critic 分支各自独立维护两层 64 维全连接隐藏层（Tanh 激活），有效抑制了离散动作 logits 的极化风险。评估方案方面，本章制定了分布外（OOD）泛化测试协议：主性能图比较四类调度策略，鲁棒性图仅比较 PPO 与 VarPPO，并在到达率梯度序列 \(\Lambda_{\text{test}} = \{0.2, 0.4, \ldots, 1.5\}\) 上检验频域打孔负载均衡奖励塑造在流量分布迁移下的影响\cite{saggese2021}。
