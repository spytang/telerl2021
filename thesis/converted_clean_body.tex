...
面对这一复杂的资源竞争环境，传统的资源分配算法面临较高的建模与求解压力。传统启发式算法（如比例公平 PF 调度）实现简单，但在应对毫秒级流量波动和多维 QoS 约束时，缺乏足够的自适应与前瞻能力\cite{dai2025}。此外，经典的数学优化方法（如混合整数非线性规划 MINLP 或连续凸近似 SCA）虽能在理论上求得较优解，然而，此类方法往往属于 NP-hard 问题，求解复杂度极高，因此难以满足 RAN 在线调度的毫秒级实时性要求\cite{feng2020, sohaib2023}。

近年来，深度强化学习（Deep Reinforcement Learning, DRL）的发展，为动态切片资源管理提供了一条值得研究的技术路径\cite{luong2019, hua2020}。DRL 能够将复杂的资源分配过程建模为 MDP，通过智能体（Agent）与网络环境的持续交互，在无需精确数学模型的情况下学习近似策略\cite{filali2022, choi2024}。利用离线训练和在线推理的特性，结合边缘计算节点的硬件加速能力，DRL 有望缓解传统优化算法在线求解开销较大的问题，并用于毫秒级实时调度场景的探索\cite{filali2022, alcaraz2023}。相关研究的实测数据表明，在合理的神经网络规模下，经过训练的 PPO 智能体执行单次资源调度的平均推理时间可控制在 0.3 至 3.0 毫秒范围内（具体取决于底层硬件算力）\cite{saggese2021}。此外，若进一步结合高度优化的 CPU/GPU 混合架构，基于深度学习的物理层调度有可能满足 5G NR 体系下低于 100μs 的严格实时性要求\cite{huang2022deluxe}。这种在线推理能力，为应对 URLLC 流量敏感的时延约束提供了可参考的技术路径\cite{huang2022deluxe, saggese2021}。基于此，如何在保障 URLLC SLA 的同时，利用 DRL 算法降低动态抢占及其物理层打孔效果对 eMBB 业务造成的冲击，兼顾其吞吐量的连续性与稳定性，是本文关注的主要研究问题\cite{alsenwi2021, saggese2021}。

同时，随着开放无线接入网（O-RAN）架构的推广，RAN 智能调度获得了更明确的工程化研究入口\cite{dai2025}。O-RAN 架构通过引入近实时无线接入网智能控制器（Near-RT RIC），并开放了标准化接口（如 E2 接口），使得诸如深度强化学习这类的智能调度器能够以 xApp 的微服务形式接入无线接入网控制流程\cite{dai2025, choi2024}。因此，在单小区下行环境中建立可复现的仿真框架，并设计与对比基于 DRL 的调度策略，可为后续面向 O-RAN 的 6G 智能调度系统设计、SLA 鲁棒性验证与工程化评估提供仿真参考\cite{saggese2021, dai2025}。
...
（1）在奖励函数塑造层面：将风险规避思想凝练为频域打孔负载均衡（Frequency-domain Puncturing Load Balancing, FPLB）。现有调度方案多以最小化系统级 eMBB 中断率为单一优化目标。然而，在复杂流量分布下，此类策略可能形成局部资源过度使用，即持续偏向少数具有高信道裕度的 eMBB 链路，进而造成用户级公平性失衡与尾部体验劣化。本文借鉴 Alsenwi 等人\cite{alsenwi2024distributed}在多智能体连续资源分配中的方差控制思想，并在纯离散抢占决策 MDP 框架内提出面向风险规避的方差惩罚策略优化算法（VarPPO）。其核心不是泛化地惩罚所有吞吐波动，而是在奖励函数中显式约束不同频域资源之间的打孔计数方差与剩余可靠性裕度方差，引导智能体在可用资源池内更均衡地分摊物理层打孔效应；码字吞吐状态与滚动吞吐方差则作为辅助稳定性指标，用于抑制由局部过度打孔引发的短期吞吐抖动。该机制旨在不增加多智能体架构复杂度的前提下，在保障 URLLC 时延约束的同时改善 eMBB 侧可靠性风险分布。
...
在打孔机制（即抢占式调度，Preemptive Scheduling）下，基站会直接中断正在进行的 eMBB 传输，将发生冲突的资源块上的 eMBB 发射功率降为零，并将其完全分配给 URLLC 数据。由于避免了复杂的干扰消除，打孔机制在满足 URLLC 严格时延方面表现较好，但代价是造成了 eMBB 数据包的部分损坏，并降低 eMBB 的吞吐量和链路可靠性。
...
在通信机制层面，本章首先对比了 eMBB 与 URLLC 在物理层与 MAC 层的底层差异：eMBB 通常以完整 Slot 为调度粒度，在 Normal CP 下每个 Slot 包含 14 个 OFDM 符号，但 Slot 绝对时长由 NR Numerology 决定并随 $\mu$ 增大而缩短；URLLC 则依托 Mini-slot 机制和更大的子载波间隔实现符号级即时调度，以满足端到端时延不超过 1 ms、可靠性达 BLER $\leq 10^{-5}$ 的约束。 在此基础上，本章进一步梳理了动态频谱复用中打孔（Puncturing）与叠加（Superposition）两种物理层机制的工作原理，重点阐述了阈值损失模型（Threshold Model）的物理抽象——将 eMBB 码字的容错上限 $C_w$ 作为智能体感知信道脆弱性的状态特征，为后续“容忍度感知抢占决策”的调度建模提供物理层依据。 针对真实网络中 URLLC 到达率的不确定性，本章还引入了风险敏感（Risk-Sensitive）调度理论，通过在优化目标中约束频域打孔负载分布或条件风险价值（CVaR），说明风险规避机制对于降低 eMBB 切片用户中断风险、改善服务等级协议（SLA）公平性具有参考意义。
...
综合论证表明，PPO 与 GAE-$\lambda$ 相结合的 Actor-Critic 架构，是本文处理离散抢占决策空间、混合业务奖励分布以及 OOD 鲁棒性评估的合理算法选择。本章的理论探讨与技术可行性分析，为第三章构建通信队列动力学模型、定义 MDP 状态与动作空间、以及推导融合风险规避指标的多目标奖励函数提供了基础。
...
本章所构建的系统模型与算法框架，为后续章节的对比实验与机理分析提供了统一且可复现的评估基准，也为 6G 超低时延网络切片调度问题的进一步仿真研究提供参考。
...
综合六个指标可以看出，\texttt{fixed\_static} 主要作为反例，展示“持续打孔同一位置”会带来严重 eMBB 破坏；\texttt{fixed\_rr} 是较强的负载均衡启发式，尤其在 puncture-count variance 上表现最好，但它不利用系统状态，因此在 eMBB outage、归一化吞吐状态方差和剩余裕度方差上仍弱于学习型策略。PPO 利用多维状态观测后，改善了 eMBB 中断控制；VarPPO 在 PPO 基础上进一步改善综合评级分数、eMBB outage rate、normalized eMBB throughput variance 和 remaining-margin variance，但提升幅度较为温和。因而，本文将 VarPPO 的贡献界定为：在不明显牺牲 URLLC 时延指标的前提下，通过频域打孔负载均衡改善 eMBB 侧可靠性风险分布，而不是声称其在所有单项指标上均优于启发式方法。
...
首先，针对``如何将 URLLC/eMBB 资源复用问题建模为 MDP''这一问题，本文将单小区下行切片的抢占调度抽象为有限时域的 tolerance-aware MDP。状态空间（State Space）刻画了 URLLC 数据包的泊松到达队列、残留容忍延迟（Deadline）、时隙位置、eMBB 打孔计数、完美 CSI 假设下的理想可靠性裕度 \(C_w\)、剩余裕度以及上一时步反馈等动态特征；动作空间（Action Space）对应切片间在具体时频资源块（RBs/FRs）上的抢占决策；奖励函数（Reward Function）则综合量化了 URLLC 时延违约惩罚和 eMBB 服务质量损失。该建模方式在本文抽象仿真环境中描述了 MAC 层资源分配的时序马尔可夫演化特征，但其性能结论应被理解为理想信道可观测条件下的理论上界。
...
第一，模型训练步数与收敛充分性仍有欠缺。作为典型的同策略（On-policy）强化学习算法，PPO 的策略梯度估计对样本吞吐量与环境交互规模具有较高依赖。受限于当前有限的训练步数（\(2 \times 10^5\) 步），智能体虽已收敛至有效策略，但尚未在高维度探索空间中充分寻优。因此，本研究仅将其界定为仿真可行性验证，而不断言其已触及调度性能上界。若后续进一步增加训练步数并优化探索-利用（exploration-exploitation）机制，智能体有望在更高负载区间内挖掘出更优的资源分配策略。

第二，奖励函数的设计依赖经验设定。虽然本文引入了 VarPPO 频域打孔负载均衡惩罚以增强负载均衡和 eMBB 传输稳定性，但各项权重仍主要来自人工设定，而未采用更系统的超参数搜索（Hyperparameter Search）或自适应多目标优化学习机制。这种固定权重配置可能导致策略在不同网络状态下呈现特定的偏好分布，限制了奖励函数在复杂场景下的可迁移性与可解释性。
...
总体而言，本文依托自建与扩展代码库，为 5G/6G 开放无线接入网的切片资源分配提供了一个可复现的深度强化学习仿真框架；该框架验证了数据驱动算法在当前抽象 SLA 约束下的调度潜力，也为后续研发面向实际基站近实时智能控制的 QoS xApp 提供了仿真层面的参考。
